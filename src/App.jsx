...
function PortfolioWidget({ widgetId, onClose, onFullscreen, isFullscreen, onCollapse, collapsed }) {
  ...
  // apiSecret intentionally not persisted — session memory only
  ...
  const [bybitBalances, setBybitBalances] = useState([]) // [{ticker, qty, usdValue}]
  ...
  const fetchPrices = useCallback(async () => {
    ...
    const allCryptoTickers = [
      ...manualCryptos.map(a => (a.ticker ?? '').toUpperCase()),
      ...bybitTickers,
    ]
    if (!allCryptoTickers.length) return
    setPricesLoading(true)
    const ids = [...new Set(allCryptoTickers.map(t => PORTFOLIO_TICKER_MAP[t] ?? t.toLowerCase()))]
    try {
      const r = await fetch(
  ...
  useEffect(() => {
    ...
    return () => clearInterval(id)
  }, [fetchPrices, assets, bybitBalances])
    ...
    try {
      ...
      const bals = coins
        ...
        .map(c => ({
          ...
          qty:      parseFloat(c.walletBalance),
  ...
  async function handleBybitConnect() {
    const key = bbApiKey.trim()
    const sec = bbApiSecret.trim()
    if (!key || !sec) return
    setBbError(null)
  ...
  function getBybitCoinPrice(ticker) {
    const cur = currency === 'EUR' ? 'eur' : 'usd'
    return prices[cgId(ticker)]?.[cur] ?? null
  }

  function getBybitCoinValue(bal) {
    const p = getBybitCoinPrice(bal.ticker)
    if (p != null) return bal.qty * p
    return bal.usdValue * (currency === 'EUR' ? 0.92 : 1)
  }

  const manualTotal  = assets.reduce((s, a) => s + getValue(a), 0)
  const bybitTotal   = bybitBalances.reduce((s, b) => s + getBybitCoinValue(b), 0)
  const totalValue   = manualTotal + bybitTotal
  ...
  const isEmpty  = assets.length === 0 && bybitBalances.length === 0
  const hasConn  = bybitConn != null
  ...
  const BROKERS = [
    ...
    { id: 'binance',    name: 'Binance',      icon: '🔶', active: false },
    { id: 'kraken',     name: 'Kraken',       icon: '🐙', active: false },
    { id: 'coinbase',   name: 'Coinbase',     icon: '🔵', active: false },
    { id: 'okx',        name: 'OKX',          icon: '⭕', active: false },
    { id: 'kucoin',     name: 'KuCoin',       icon: '🟢', active: false },
        ...
        {isEmpty ? (
          ...
          <div className="pf-overview">
            ...
            <div className="pf-overview-footer">
              <span>{assets.length + bybitBalances.length} asset{(assets.length + bybitBalances.length) !== 1 ? 's' : ''}</span>
          ...
          <div className="pf-holdings">
              ...
              <table className="pf-table">
                ...
                <tbody>
                  ...
                  {bybitBalances.map(bal => {
                    const price = getBybitCoinPrice(bal.ticker)
                    const value = getBybitCoinValue(bal)
                    return (
                      <tr key={`bybit-${bal.ticker}`}>
                        <td className="pf-col-name">{bal.ticker}</td>
                        <td className="pf-col-ticker">{bal.ticker}</td>
                        <td>
                          <span className="pf-type-badge" style={{ color: '#00D4FF', borderColor: '#00D4FF' }}>Crypto</span>
                        </td>
            ...
            <div className="pf-holdings-footer">
              {lastUpdated && <span>Updated {fmtUpdated()}</span>}
              {bybitLoading && <span className="pf-spinner" />}
              <span style={{ marginLeft: 'auto', color: 'var(--text-muted)', fontSize: '10px' }}>via CoinGecko</span>
            </div>
          </div>
        )}
              ...
              <div className="pf-broker-grid">
                {BROKERS.map(b => (
                  <div
                    ...
                    onClick={b.active ? openBybitModal : undefined}
            ...
            <div className="pf-bybit-body" onPointerDownCapture={e => e.stopPropagation()}>
              ...
              <div className="pf-form-row">
                ...
                <input
                  ...
                  type="password"
                  value={bbApiSecret}
                  onChange={e => setBbApiSecret(e.target.value)}
