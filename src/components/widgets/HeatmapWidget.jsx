...
export default function HeatmapWidget({ onClose, onFullscreen, isFullscreen, onCollapse, collapsed, workspacePaused = false }) {
  ...
  useEffect(() => {
    if (iframeRef.current) {
      iframeRef.current.src = workspacePaused ? '' : HEATMAP_URL
    }
  }, [workspacePaused])
  ...
  return (
    <div className="widget" data-collapsed={collapsed || undefined}>
      ...
      <iframe ref={iframeRef} src={workspacePaused ? '' : HEATMAP_URL} style={{ flex: 1, width: '100%', minHeight: 0, border: 'none', display: 'block' }} title="Market Heatmap" allow="clipboard-write" />
    </div>
  )
}
