...
async function fetchSub(name) {
  const feed = `https://www.reddit.com/r/${name}/.rss`
  const res = await fetch(`/api/rss?url=${encodeURIComponent(feed)}`, { signal: AbortSignal.timeout(12000) })
  const json = await res.json().catch(() => null)
  if (res.status === 429 || json?.status === 'rate_limited') throw new Error('rate_limited')
  if (!json || json.status !== 'ok') throw new Error(json?.error || 'rss error')
...
export default function SocialFeedWidget({ paused, config, onSaveConfig }) {
  ...
  function saveSubs(next) { patch({ subs: next }) }

  const fetchAll = useCallback(async () => {
    const enabled = subsRef.current.filter(s => s.enabled)
    if (!enabled.length) { setLoading(false); setPosts([]); return }
    setLoading(true)
    const results = await Promise.allSettled(enabled.map(s => fetchSub(s.name).then(posts => ({ subId: s.id, posts }))))
    const newErrors = {}; const allPosts = []
    results.forEach((r, i) => {
  ...
  function addSub() {
    ...
    if (!name) return
    if (subs.some(s => s.name.toLowerCase() === name)) { setNewSubInput(''); setAddingSub(false); return }
          ...
          <div className="rss-source-list">
            {subs.map(s => (
              <div key={s.id} className="rss-source-item">
                ...
                {errorBySub[s.id] && <span className="rss-source-err" title="Could not load">⚠</span>}
                <button className="rss-source-del" onClick={() => removeSub(s.id)} title="Remove">×</button>
              </div>
            ))}
            {addingSub && (
              <div className="rss-add-source-form" onPointerDownCapture={e => e.stopPropagation()} style={{ padding: '6px 8px', borderTop: '1px solid #1a2535' }}>
                <input autoFocus className="rss-add-source-input" value={newSubInput} onChange={e => setNewSubInput(e.target.value)}
                  onKeyDown={e => { if (e.key === 'Enter') addSub(); if (e.key === 'Escape') { setAddingSub(false); setNewSubInput('') } }}
      ...
      <div className="rss-right" style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
        {enabledSubs.length > 0 && (
          <div className="rss-filters-strip" onPointerDownCapture={e => e.stopPropagation()}>
            <div className="rss-filters-chips">
              {enabledSubs.map(s => (
                <div key={s.id} className={`rss-filter-chip${!hiddenSubs.has(s.id) ? ' active' : ''}`} onClick={() => toggleSubVisible(s.id)}>
        ...
        <div className="rss-filter-bar" onPointerDownCapture={e => e.stopPropagation()}>
          <input className="rss-input rss-filter-input" value={filterInput}
            onChange={e => { const v = e.target.value; setFilterInput(v); clearTimeout(filterTimerRef.current); filterTimerRef.current = setTimeout(() => setFilter(v), 150) }}
            placeholder="Filter posts…" />
          {filterInput && <button className="rss-filter-clear" onClick={() => { setFilterInput(''); setFilter('') }} title="Clear">×</button>}
        </div>
        ...
        <div className="rss-articles" style={{ flex: 1, overflowY: 'auto', minHeight: 0 }}>
          {isFirstLoad ? (
            ...
            <div className="empty-state">
              ...
              {filter ? `No posts matching "${filter}"` : subs.length === 0 ? 'Add subreddits to follow' : 'No posts available'}
            </div>
          ) : (
            displayPosts.map((post, i) => (
              <a key={i} className="rss-article" href={post.link} target="_blank" rel="noopener noreferrer">
                <div className="rss-art-body">
                  ...
                  <div style={{ color: 'var(--text-secondary)', fontSize: '10px', marginTop: 2 }}>{post.author}</div>
                </div>
